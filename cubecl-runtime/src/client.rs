...
impl<R: Runtime> ComputeClient<R> {
    ...
    /// inside it. No allocation and no copy happen — the GPU reads the host pages
    /// directly (unified memory). See [`ComputeServer::register_external_aliased`].
    ///
    /// # Safety
    /// `ptr` must point to a live, page-aligned host region of at least `page_len`
    /// bytes that remains valid and immutable for the entire lifetime of the
    /// returned handle (and any tensor built from it).
    pub unsafe fn register_external_aliased(
        &self,
        ...
        // SAFETY: caller guarantees the region stays valid; we only read it.
        unsafe impl Send for SendPtr {}
        let p = SendPtr(ptr);
        self.device
            .submit_blocking(move |server| {
                // Rebind the whole `p` so the closure captures `SendPtr` as a unit
