...
impl DeviceService for CudaServer {
    fn init(device_id: cubecl_common::device::DeviceId) -> Self {
        ...
        // CUBECL_MEMORY_CONFIG selects the allocation strategy without a
        // rebuild. `exclusive` gives each allocation its own page;
        // `subslices` (the default) packs allocations into shared pages sized
        // from cuDeviceTotalMem/4, which strands whatever cannot be packed.
        // An unknown value is a hard error rather than a silent fallback: a
        // typo that quietly leaves the default in place would make an A/B
        // measurement report "no difference" for the wrong reason.
        match std::env::var("CUBECL_MEMORY_CONFIG").as_deref() {
            Ok("exclusive") => options.memory_config = MemoryConfiguration::ExclusivePages,
            #[cfg(not(exclusive_memory_only))]
            Ok("subslices") => options.memory_config = MemoryConfiguration::SubSlices,
            Ok(other) => panic!(
                "CUBECL_MEMORY_CONFIG={other:?} is not recognised (expected \"exclusive\" or \"subslices\")"
            ),
            Err(_) => {}
        }
